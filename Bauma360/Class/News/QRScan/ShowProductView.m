    ...
    if (product.categoryName == nil){
        ...
        [category fetchIfNeededInBackgroundWithBlock:^(AVObject *object, NSError *error) {
            if (!error) {
                ...
                self->lblSize.text = [NSString stringWithFormat:@"产品尺寸：%d*%d*%d(mm)", category.length, category.width, category.height];
                self->lblWeight.text = [NSString stringWithFormat:@"产品重量：%d(kg)", category.weight];
        ...
        [vendor fetchIfNeededInBackgroundWithBlock:^(AVObject *object, NSError *error) {
            if (!error){
                ...
                self->lblSmCode.text = [NSString stringWithFormat:@"标签序号：%@%@%05d", vendor.vendorCode, sy, rfid.printingSN];
            }
        }];
    }
}
